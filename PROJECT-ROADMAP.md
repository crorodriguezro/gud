...
| `E1-T04` Implement long-lived sequential multi-frame receive | P0 | verified | E1-T03 | Production `status-on-set-aio` completed the declared 100-frame OnePlus 6/Pi Zero 2 W gate at 12,800 bytes with one exact AIO receive per transaction, ordered sequence IDs 1-100, aggregate `Processing -> Idle` before readmission, final Idle ownership, and zero poison, timeout, host failure, DWC2 anomaly, or kernel fault. Evidence: `../gud-gadget/evidence/functionfs-status-on-set-e1-t04-hs-rerun-20260818T022645Z/`. |
