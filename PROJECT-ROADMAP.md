...
| `E1-T05` Disconnect, suspend, timeout, and failure matrix | P0 | blocked | E1-T04 | Offline containment coverage and the matrix are defined in `../gud-gadget/docs/e1-t05-lifecycle-matrix-runbook.md`. The 2026-08-18 hardware preflight reached both targets but stopped before a case because T05 source was uncommitted, worktrees were dirty, and OnePlus sudo was unavailable. Ten normal reconnects plus controlled suspend/error cases must still preserve state invariants; only proven Idle cleans up normally, and accepted-I/O, Processing, or Poisoned ownership never uses unsafe software teardown. |
