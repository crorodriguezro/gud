...
# E1-T04 host half: one module load and 100 ordered atomic commits. This is a
# bounded long-lived production gate, deliberately distinct from T01/T02.
...
evidence_dir=${EVIDENCE_DIR:-$script_dir/local/evidence/status-on-set-t04-$(date -u +%Y%m%d-%H%M%S)}
readonly payload_length=12800
readonly transaction_count=100
...
echo XDISP_T04_BEGIN payload_bytes=\$payload_length transaction_count=\$transaction_count > /dev/kmsg
...
for transaction in \$(seq 1 \$transaction_count); do
  start_ns=\$(date +%s%N)
  echo XDISP_T04_TRANSACTION_\${transaction}_START payload_bytes=\$payload_length start_ns=\$start_ns > /dev/kmsg
  timeout 10s /tmp/gud-kms-stage atomic-commit /dev/dri/card1
  command_result=\$?
  end_ns=\$(date +%s%N)
  echo XDISP_T04_TRANSACTION_\${transaction}_END command_result=\$command_result end_ns=\$end_ns > /dev/kmsg
  printf 'XDISP_T04_HOST_RESULT transaction=%s payload_bytes=%s command_result=%s start_ns=%s end_ns=%s\n' \\
    "\$transaction" "\$payload_length" "\$command_result" "\$start_ns" "\$end_ns"
  test "\$command_result" -eq 0 || { overall=\$command_result; break; }
done
echo XDISP_T04_END result=\$overall > /dev/kmsg
exit "\$overall"
EOF
...
{
    printf 'host_runner=xdisp-t04-status-on-set-100-frame\n'
    printf 'configured_transaction_count=%s\n' "$transaction_count"
    printf 'configured_payload_bytes=%s\n' "$payload_length"
    printf 'remote_runner_result=%s\n' "$remote_runner_result"
    grep -E 'XDISP_T04_HOST_RESULT' "$evidence_dir/phone-stdout.log" || true
    grep -E 'XDISP_T04_(BEGIN|TRANSACTION_[0-9]+_(START|END)|END)|XDISP_PROBE (start|complete)' \
